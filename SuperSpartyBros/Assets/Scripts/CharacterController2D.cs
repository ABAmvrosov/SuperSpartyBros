...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		// allow douable jump after grounded
		if (_isGrounded) {
			_canDoubleJump = true;
		}
		...
		if (_isGrounded && CrossPlatformInputManager.GetButtonDown ("Jump")) { // If grounded AND jump button pressed, then allow the player to jump
			DoJump ();
		} else if (_canDoubleJump && CrossPlatformInputManager.GetButtonDown ("Jump")) { // If grounded AND jump button pressed, then allow the player to jump 
			DoJump ();
